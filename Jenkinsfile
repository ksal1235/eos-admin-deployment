 def label = "eosagent"
  ...
  """
      ...
      node (label) {
          ...
          stage ('Checkout SCM'){
            git credentialsId: 'git', url: 'https://github.com/ksal1235/eos-admin-deployment.git', branch:  "${env}"
          }

          stage ('Helm Chart') {
            container('build') {
                withCredentials([usernamePassword(credentialsId: 'jfrog', usernameVariable: 'username', passwordVariable: 'password')]) {
                      sh '/usr/local/bin/helm repo add eos-helm-local  https://shoebcloudadmin.jfrog.io/artifactory/eos-helm-local --username $username --password $password'
